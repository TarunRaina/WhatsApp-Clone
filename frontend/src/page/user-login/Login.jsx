  ...
  return (
    <div className={`min-h-screen ${theme === 'dark' ? 'bg-gray-900' : 'bg-gradient-to-br from-green-400 to-blue-500'} flex items-center justify-center p-4 overflow-hidden`}>
      ...
      >
        ...
        {step === 1 && (
          <form onSubmit={handleLoginSubmit(onPhoneSubmit)} className="space-y-4">
        ...
        {step === 2 && (
          <form onSubmit={handleOtpSubmit(onOtpSubmit)} className="space-y-4">
            <p className={`text-center ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'} mb-4`}>
              Please enter the 6-digit OTP send to {userPhoneData?.phoneSuffix} {userPhoneData?.phoneNumber}
            </p>
        ...
        {step === 3 && (
          <form onSubmit={handleProfileSubmit(onProfileSubmit)} className="space-y-4">
            ...
            <div className="flex items-center space-x-2">
              <input
                {...profileRegister("agreed", { required: "You must agree to the terms" })}
                type="checkbox"
                id="terms"
              ...
              <label htmlFor="terms" className={`text-sm ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>
                I agree to the <a href="#" className="text-green-500 hover:underline">Terms and Conditions</a>
              </label>
            </div>
            {profileErrors.agreed && <p className="text-red-500 text-sm">{profileErrors.agreed.message}</p>}
            <button
              type="submit"
